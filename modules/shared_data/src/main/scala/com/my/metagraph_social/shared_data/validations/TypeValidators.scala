...
object TypeValidators {
  ...
  def validateIfParentPostExists(
    ...
    ParentPostNotExists.unlessA(calculatedState.users.get(userId).exists(_.posts.exists(_.postId == postId)))
  ...
  def validateIfCommentExists(
    ...
    val userPosts = calculatedState.users.get(userId).map(_.posts).getOrElse(List.empty)
