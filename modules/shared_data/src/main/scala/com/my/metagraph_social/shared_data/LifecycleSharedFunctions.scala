  ...
  def validateData[F[_] : Async : JsonSerializer](
    ...
    updates.traverse { signedUpdate =>
      signedUpdate.value match {
        ...
        case _: DeleteComment =>
          val signedDeleteComment = signedUpdate.asInstanceOf[Signed[DeleteComment]]
          deleteCommentValidationL0(signedDeleteComment, state.calculated)
      }
    }.map(_.reduce)
  }
            ...
            .flatMap(newState => {
              updates.foldLeftM(newState) { (acc, signedUpdate) =>
                ...
                signedUpdate.value match {
                  ...
                  case _: DeleteComment =>
                    val signedDeleteComment = signedUpdate.asInstanceOf[Signed[DeleteComment]]
                    combineDeleteComment(signedDeleteComment, acc)
                }
